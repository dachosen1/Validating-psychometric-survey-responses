...
# Pre-processing, loading the data
...
radio_DF['aspect_ratio'] = [str(radio_DF['window_x'][i]) + "x" + str(radio_DF['window_y'][i]) for i in
                            range(len(radio_DF))]
...
def distance(df):
    ...
    for i in range(len(clients)):
        ...
        for j in range(len(per_id)):
            ...
            while m < len(y):
                dist = round(sqrt(
                    (y.cord_x.iloc[m] - y.cord_x.iloc[m - 1]) ** 2 + (y.cord_y.iloc[m] - y.cord_y.iloc[m - 1]) ** 2),
                             0)  # distance formula between two points.
                pl.append(dist)
                m += 1
    return pl
...
rr_df.insert(loc = rr_df.columns.get_loc('distance') + 1, column = 'direction',
             value = ["Nada" for val in rr_df.distance])
