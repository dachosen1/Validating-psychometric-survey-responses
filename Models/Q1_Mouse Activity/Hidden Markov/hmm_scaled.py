        ...
        for it in range(max_iter):
            ...
            for n in range(N):
                ...
                for t in range(T - 2, -1, -1):
                    beta[t] = self.state_transition_matrix.dot(self.output_distribution[:, x[t + 1]] * beta[t + 1]) / \
                              scale[t + 1]
                betas.append(beta)
            ...
            for n in range(N):
                ...
                for i in range(self.hidden_states):
                    for j in range(self.hidden_states):
                        for t in range(T - 1):
                            a_num[i, j] += alphas[n][t, i] * betas[n][t + 1, j] * self.state_transition_matrix[i, j] * \
                                           self.output_distribution[j, x[t + 1]] / \
                                           scales[n][t + 1]
